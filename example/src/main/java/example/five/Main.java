...
public class Main {
  ...
  public static void main(String[] args)
  {
    List<Datum> data = List.of(
      new Datum(1, false, Product.CONFLUENCE, "cloud"),
      new Datum(2, true, Product.JIRA, "server"),
      new Datum(3, false, Product.JIRA, "dataCentre"),
      new Datum(4, true, Product.STASH, "dataCentre"));
    ...
    //   Monoid.product(sum, sum, CountTotal::make, CountTotal::getCount, CountTotal::getTotal));
    CountTotal countTotal = data.foldMapFilter(Datum::isCompleted, d -> new CountTotal(d.getTime()),
      Monoid.product(sum, sum, CountTotal::make, CountTotal::getCount, CountTotal::getTotal));
    System.out.println(
      "total = " + countTotal.getTotal() + ", count = " + countTotal.getCount() + ", mean = " + countTotal.mean());

    Function<Product, Long> timePerProduct = data.foldMap(
      d -> (p -> (p == d.getProduct()) ? d.getTime() : sum.zero()),
